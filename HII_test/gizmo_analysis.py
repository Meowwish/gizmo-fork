...
boltzmann_cgs = 1.38066e-16  # erg K^{-1}
m_H = 1.6726e-24    # g
...
# code units
unittime_cgs = 3.085678e16   # s  (0.978 Gyr in s)
unitlength_cgs = 3.085678e18 # cm (1 parsec)
unitmass_cgs = 1.989e33      # g  (1 Msun)
unitbfield_cgs = 1.0e4       # gauss (1 Tesla)
...
def plot_stars_on_axis(ax, star_coords, xymin=0., xymax=10., s=0.05):
    if star_coords is not None:
        x = star_coords[:, 0]
        y = star_coords[:, 1]
        xmask = np.logical_and(x > xymin, x < xymax)
        ymask = np.logical_and(y > xymin, y < xymax)
        mask = np.logical_and(xmask, ymask)
        ax.scatter(x[mask], y[mask], s=s, color='black')


def save_slice_plot(mesh, field, filename, colorbar_label="",
                    bfield=None, norm=None,
                    star_coords=None, xymin=0., xymax=10., plane='z', vmin=10., vmax=1.0e7):
    res = 200
    x = y = np.linspace(xymin, xymax, res)
    L = xymax - xymin
    center = np.array([xymin + 0.5*L, xymin + 0.5*L, xymin + 0.5*L])

    field_slice = mesh.Slice(field, center=center, size=L, res=res, plane=plane)

    fig, ax = plt.subplots(figsize=(6, 6))
    if norm is None:
        norm = colors.LogNorm(vmin=vmin, vmax=vmax)
    
    p = ax.imshow(field_slice, cmap='viridis',
    ...
    if bfield is not None:
        ...
        x = y = np.linspace(xymin, xymax, bfield_res)
    ...
    if star_coords is not None:
        plot_stars_on_axis(ax, star_coords, xymin=xymin, xymax=xymax)
